...
public class FileService {
    ...
    String uploadPath = "C:\\Users\\codemaster\\Documents\\workspace\\tj_2024A_spring\\src\\main\\resources\\static\\upload\\";

    // [1] 파일 업로드
    // 매개변수로 파일의바이트가 저장된 MultipartFile 인터페이스
    // 업로드 된 파일명 반환
    public String fileUpload(final MultipartFile multipartFile) {
        ...
        final String uuid = UUID.randomUUID().toString(); // 난수의 UUID 생성 , 임의의 UUID 규약에 따른 문자열 생성
    ...
    @Autowired
    private HttpServletRequest request; // HTTP 요청 객체  , HTTP로 요청 들어온 정보 와 기능이 포함된 객체
    @Autowired
    private HttpServletResponse response; // HTTP 응답 객체 , HTTP로 응답 할때 정보 와 기능이 포함된 객체

    // [2] 파일 다운로드
    public void fileDownLoad(final String filename) {
        ...
        try {
            ...
            final FileInputStream fin = new FileInputStream(downLoadPath);
            // 2-2 파일의 용량만큼 배열 선언
            // 파일의 용량
            ...
            final ServletOutputStream fout = response.getOutputStream();
